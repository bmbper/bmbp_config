...
const PageTreeView = () => {
    const TreeNodExtraAction = (props) => {
        ...
        return (<arco.Menu style={{width: '100px', background: '#fff', border: '1px solid #e8e8e8'}}>
            <arco.Menu.Item key="addBrother" onClick={() => {
                PageAction.addBrotherNode(data);
            }}>新增同级
            </arco.Menu.Item>
            <arco.Menu.Item key="addChild" onClick={() => {
                PageAction.addChildNode(data);
            }}>新增子级
            </arco.Menu.Item>
            <arco.Menu.Item key="edit" onClick={() => {
                PageAction.editNode(data);
            }}>编辑
            </arco.Menu.Item>
            <arco.Menu.Item key="remove" onClick={() => {
                PageAction.removeNode(data);
            }}>删除
            </arco.Menu.Item>
            <arco.Menu.Item key="enable" onClick={() => {
                PageAction.enableNode(data);
            }}>启用
            </arco.Menu.Item>
            <arco.Menu.Item key="disable" onClick={() => {
                PageAction.disableNode(data);
            }}>停用
            </arco.Menu.Item>
            <arco.Menu.Item key="changeParent" onClick={() => {
    ...
    const renderTreeNodExtra = (node) => {
        return (
            <arco.Dropdown droplist={<TreeNodExtraAction dataRef={node.dataRef}/>} position='bl'>
                <arcoicon.IconMore
                    style={{
                        position: 'absolute',
                        right: 8,
    ...
    return (<div>
        <div style={{display: 'block'}}>
            <arco.Input.Search searchButton placeholder="请输入" onSearch={(v) => {
                PageAction.findTreeData(v)
            }}/>
        </div>
        <arco.Tree treeData={PageState.treeData} blockNode={true} renderExtra={renderTreeNodExtra}
                   onSelect={(keys, extra) => {
...
const PageGridView = () => {
    return (<div className="bmbp-grid-container"><PageGridSearchForm/><PageGridToolBar/><PageGridTable/><PageGridPage/>
    </div>)
}
const PageGridSearchForm = () => {
    return (<div className="bmbp-grid-search">
        <arco.Form colon ref={PageState.searchFormRef}>
            <arco.Grid.Row guides={[1, 1]}>
                <arco.Grid.Col span={7}>
                    <arco.Form.Item label="别名" field="dictAlias">
                        <arco.Input placeholder="请输入别名"/>
                    </arco.Form.Item>
                </arco.Grid.Col>
                <arco.Grid.Col span={7}>
                    <arco.Form.Item label="名称" field="dictName">
                ...
                <arco.Grid.Col span={7}>
                    <arco.Form.Item label="状态" field="dataStatus">
                        <arco.Select placeholder="请选择状态">
                            <arco.Select.Option key={'1'} value={'1'}>已启用</arco.Select.Option>
                ...
                <arco.Grid.Col span={3}>
                    <arco.Form.Item>
                        <arco.Space>
                            <arco.Button type="primary" style={{marginLeft: '8px'}} onClick={() => {
...
const PageGridToolBar = () => {
    ...
    return (<div className="bmbp-grid-toolbar">
            <div className="bmbp-grid-toolbar major">
                {
                    PageState.selectTreeNodeData ? <arco.Button type="primary" onClick={() => {
                        PageAction.addChildNode(PageState.selectTreeNodeData.dataRef)
                    }}>新增</arco.Button> : null
                }
                {
                    PageState.selectedRowKeys && PageState.selectedRowKeys.length > 0 ? <arco.Button type="primary" status='danger' onClick={() => {
...
const PageGridTable = () => {
    const enableAction = (record) => {
        return [
            <arco.Tooltip content="新增子级">
                <arco.Button type="primary" status='danger' icon={<arcoicon.IconDelete/>} size="mini"
                             onClick={() => {
                             }}></arco.Button>
            </arco.Tooltip>,
            <arco.Tooltip content="查看">
                <arco.Button type="secondary" icon={<arcoicon.IconView/>} size="mini" onClick={() => {
                }}></arco.Button>
            </arco.Tooltip>,
            <arco.Tooltip content="停用">
                <arco.Button type="secondary" icon={<arcoicon.IconEdit/>} size="mini" onClick={() => {
                }}></arco.Button>
            </arco.Tooltip>,
    ...
    const disableAction = (record) => {
        return [
            <arco.Tooltip content="编辑">
                <arco.Button type="primary" icon={<arcoicon.IconEdit/>} size="mini" onClick={() => {
                    PageAction.editNode(record);
                }}></arco.Button>
            </arco.Tooltip>,
            <arco.Tooltip content="启用">
                <arco.Button type="primary" icon={<arcoicon.IconPlayArrow/>} size="mini" onClick={() => {
                }}></arco.Button>
            </arco.Tooltip>,
            <arco.Tooltip content="变更上级">
                <arco.Button type="secondary" icon={<arcoicon.IconEdit/>} size="mini" onClick={() => {
            ...
            <arco.Tooltip content="新增子级">
                <arco.Button type="secondary" icon={<arcoicon.IconPlus/>} size="mini" onClick={() => {
                    PageAction.addChildNode(record);
                }}></arco.Button>
            </arco.Tooltip>,
            <arco.Popconfirm focusLock title='删除确认'
    ...
    return (<div className="bmbp-grid-table">
        <arco.Table columns={gridColumn} data={PageState.gridData}
...
const PageGridPage = () => {
    return (<div className="bmbp-grid-page">
        <arco.Pagination size={PageState.showPageSize} total={PageState.pageData.total} showTotal showJumper
                         sizeCanChange
