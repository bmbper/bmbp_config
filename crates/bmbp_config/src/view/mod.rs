...
#[handler]
pub async fn dict_view(_: &mut Request, res: &mut Response) {
    let mut ctx = ctx_init();
    res.render(Text::Html((*CTX_TERA).render("dict.html", &ctx).unwrap()));
}
...
pub async fn security_view(_: &mut Request, res: &mut Response) {
    let mut ctx = ctx_init();
    res.render(Text::Html((*CTX_TERA).render("security.html", &ctx).unwrap()));
}
