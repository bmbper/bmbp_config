...
use crate::action::dict::bean::{
    BatchComboVo, BatchReqVo, BmbpCombo, BmbpCombos, BmbpDict, BmbpDictColumn, BmbpDisplay,
};

pub struct BmbpDictService;

impl BmbpDictService {
    pub(crate) async fn find_dict_tree(
        depot: &mut Depot,
        params: &BmbpDict,
    ) -> BmbpResp<Option<Vec<BmbpDict>>> {
    ...
    pub(crate) async fn find_dict_page(
        depot: &mut Depot,
        page_req: &BmbpPageReq<BmbpDict>,
    ) -> BmbpResp<Option<PageData<BmbpDict>>> {
        let query_wrapper = Self::build_dict_query_wrapper(depot, page_req.get_params()).await?;
        let orm = parse_orm(depot)?;
        BmbpCurdDao::execute_query_page::<BmbpDict>(
            orm,
    ...
    pub(crate) async fn find_dict_list(
        depot: &mut Depot,
        params: &BmbpDict,
    ) -> BmbpResp<Option<Vec<BmbpDict>>> {
    ...
    pub(crate) async fn find_dict_tree_ignore(
        depot: &mut Depot,
        params: &BmbpDict,
    ) -> BmbpResp<Option<Vec<BmbpDict>>> {
        ...
        if dict_vec.is_some() {
            Ok(Some(BmbpTreeUtil::build_tree::<BmbpDict>(
                dict_vec.unwrap(),
            )))
        } else {
            Ok(None)
    ...
    pub(crate) async fn find_dict_info(
        depot: &mut Depot,
    ...
    pub(crate) async fn find_dict_info_in_alias(
        depot: &mut Depot,
        alias: &[String],
    ) -> BmbpResp<Option<Vec<BmbpDict>>> {
        if alias.is_empty() {
    ...
    pub(crate) async fn insert_dict(
        depot: &mut Depot,
        params: &mut BmbpDict,
    ) -> BmbpResp<Option<BmbpDict>> {
        ...
        if params.get_dict_alias().as_ref().is_none()
            || params.get_dict_alias().as_ref().unwrap().is_empty()
        {
            return Err(BmbpRespErr::err(
                Some("VALID".to_string()),
                Some("请传入字典别名".to_string()),
            ));
        }
        if params.get_dict_name().as_ref().is_none()
            || params.get_dict_name().as_ref().unwrap().is_empty()
        {
            return Err(BmbpRespErr::err(
                Some("VALID".to_string()),
            ...
            || params.get_dict_value().as_ref().unwrap().is_empty()
        {
            return Err(BmbpRespErr::err(
                Some("VALID".to_string()),
                Some("请传入字典值".to_string()),
            ));
        }
        if params.get_dict_parent_code().as_ref().is_none()
            ...
            Self::find_dict_info_by_code(depot, params.get_dict_parent_code().as_ref()).await?
        {
            ...
            params.set_dict_code_path(Some(format!(
                "{},{},",
                params.get_dict_parent_code().as_ref().unwrap(),
                dict_code
            )));
            params.set_dict_name_path(Some(format!(
                "{},{},",
                params.get_dict_parent_code().as_ref().unwrap(),
                dict_name
            )));
        ...
        // 校验别名是否重复
        Self::check_save_alias(
            orm.unwrap(),
            params.get_dict_alias().clone().as_ref().unwrap(),
            params.get_data_id().clone(),
        )
        .await?;
        Self::check_save_name(
            orm.unwrap(),
            params.get_dict_parent_code().clone().unwrap(),
            params.get_dict_name().clone().unwrap(),
            params.get_data_id().clone(),
        )
        .await?;
        Self::check_save_value(
            orm.unwrap(),
            params.get_dict_parent_code().clone().unwrap(),
            params.get_dict_value().clone().unwrap(),
            params.get_data_id().clone(),
        )
        .await?;
        ...
        insert_wrapper.insert_column_value(
            BmbpDictColumn::DictParentCode.get_ident(),
            params.get_dict_parent_code().as_ref().unwrap(),
        );
        insert_wrapper.insert_column_value(
            BmbpDictColumn::DictName.get_ident(),
            params.get_dict_name().as_ref().unwrap(),
        );
        insert_wrapper.insert_column_value(
            BmbpDictColumn::DictCodePath.get_ident(),
            params.get_dict_code_path().as_ref().unwrap(),
        );
        insert_wrapper.insert_column_value(
            BmbpDictColumn::DictNamePath.get_ident(),
            params.get_dict_name_path().as_ref().unwrap(),
        );
        insert_wrapper.insert_column_value(
            BmbpDictColumn::DictTreeGrade.get_ident(),
            params.get_dict_tree_grade().unwrap_or(1),
        );
        insert_wrapper.insert_column_value(
            BmbpDictColumn::DictAlias.get_ident(),
            params.get_dict_alias().as_ref().unwrap(),
        );
        insert_wrapper.insert_column_value(
            BmbpDictColumn::DictValue.get_ident(),
            params.get_dict_value().as_ref().unwrap(),
        );

        insert_wrapper.insert_column_value(
            BmbpDictColumn::DataId.get_ident(),
            params.get_data_id().as_ref().unwrap(),
        );
        insert_wrapper.insert_column_value(
            BmbpDictColumn::DataLevel.get_ident(),
            params.get_data_level().clone().unwrap_or("0".to_string()),
        );
        insert_wrapper.insert_column_value(
            BmbpDictColumn::DataFlag.get_ident(),
            params.get_data_flag().clone().unwrap_or("0".to_string()),
        );
        insert_wrapper.insert_column_value(
            BmbpDictColumn::DataSort.get_ident(),
        ...
        insert_wrapper
            .insert_column_value(BmbpDictColumn::DataUpdateTime.get_ident(), current_time());
        let current_user = match user {
            Some(u) => u.get_id().clone().unwrap_or("".to_string()),
            None => "".to_string(),
        };
        insert_wrapper.insert_column_value(
            BmbpDictColumn::DataCreateUser.get_ident(),
            current_user.clone(),
        );
        insert_wrapper.insert_column_value(
            BmbpDictColumn::DataUpdateUser.get_ident(),
            current_user.clone(),
        );
    ...
    pub(crate) async fn update_dict(
        depot: &mut Depot,
        params: &mut BmbpDict,
    ) -> BmbpResp<Option<BmbpDict>> {
        if params.get_data_id().is_none() {
        ...
        update_wrapper.set(
            ...
            params.get_dict_tree_grade().unwrap(),
        );
        ...
        if &old_dict_name != params.get_dict_name().as_ref().unwrap()
            || &old_dict_parent_code != params.get_dict_parent_code().as_ref().unwrap()
        {
            Self::update_children_dict_path(
                orm,
                &old_dict_code_path,
                &dict_code_path,
                &old_dict_name_path,
    ...
    pub(crate) async fn enable_dict(
        depot: &mut Depot,
        params: Option<&String>,
    ) -> BmbpResp<Option<u64>> {
        let orm = parse_orm(depot)?;
        let dict_info: Option<BmbpDict> =
            BmbpCurdService::find_info_by_id::<BmbpDict>(orm, params).await?;
        if dict_info.is_none() {
            return Err(BmbpRespErr::err(
        ...
        update_wrapper.in_v(BmbpDictColumn::DictCode, code_vec);
        BmbpCurdDao::execute_update::<BmbpDict>(orm, &update_wrapper).await
    }

    pub(crate) async fn batch_enable_dict(
        depot: &mut Depot,
        params: &BatchReqVo,
    ) -> BmbpResp<Option<u64>> {
        let mut u64 = 0;
        if let Some(ids) = params.get_ids() {
            for id in ids {
                let resp = Self::enable_dict(depot, Some(id)).await?;
                if resp.is_some() {
                    u64 += resp.unwrap();
                }
            }
    ...
    pub(crate) async fn disable_dict(
        depot: &mut Depot,
        params: Option<&String>,
    ) -> BmbpResp<Option<u64>> {
        let orm = parse_orm(depot)?;
    ...
    pub(crate) async fn batch_disable_dict(
        depot: &mut Depot,
        params: &BatchReqVo,
    ) -> BmbpResp<Option<u64>> {
        let mut u64 = 0;
        if let Some(ids) = params.get_ids() {
            for id in ids {
                let resp = Self::disable_dict(depot, Some(id)).await?;
                if resp.is_some() {
                    u64 += resp.unwrap();
                }
            }
    ...
    pub(crate) async fn remove_dict(
        depot: &mut Depot,
        params: Option<&String>,
    ) -> BmbpResp<Option<u64>> {
        let orm = parse_orm(depot)?;
    ...
    pub(crate) async fn find_dict_combo(
        depot: &mut Depot,
        alias: Option<&String>,
        cascade: Option<&String>,
    ) -> BmbpResp<Option<Vec<BmbpCombo>>> {
        ...
        match dict_combo {
            Some(v) => Ok(Some(
                v.get(&alias.unwrap().clone()).unwrap_or(&vec![]).clone(),
            )),
            None => Ok(None),
        }
    }

    pub(crate) async fn find_dict_combos(
        depot: &mut Depot,
        params: &BatchComboVo,
    ) -> BmbpResp<Option<BmbpCombos>> {
    ...
    pub(crate) async fn find_dict_display(
        depot: &mut Depot,
        alias: Option<&String>,
        cascade: Option<&String>,
    ) -> BmbpResp<Option<BmbpDisplay>> {
    ...
    pub(crate) async fn find_dict_displays(
        depot: &mut Depot,
        params: &BatchComboVo,
    ) -> BmbpResp<Option<BmbpDisplay>> {
    ...
    async fn build_dict_query_wrapper(
        depot: &mut Depot,
        params_op: Option<&BmbpDict>,
    ...
    async fn find_dict_children_by_alias(
        depot: &mut Depot,
        alias: &[String],
        cascade: Option<&String>,
    ) -> BmbpResp<Vec<BmbpDict>> {
        ...
        let orm = parse_orm(depot)?;
        let dict_vec = BmbpCurdDao::execute_query_list::<BmbpDict>(orm, &query_wrapper).await?;
        Ok(BmbpTreeUtil::build_tree::<BmbpDict>(
            dict_vec.unwrap_or(vec![]),
        ))
    }
    fn convert_dict_to_combos(dict_vec: Vec<BmbpDict>) -> BmbpResp<Option<BmbpCombos>> {
        ...
        for dict in dict_vec.as_slice() {
            ...
            let combo_vec =
                Self::convert_to_vec(dict.get_dict_children().as_ref().unwrap().as_slice());
    ...
    async fn update_children_dict_path(
        orm: &RdbcOrm,
        ...
        update
            .table(BmbpDict::get_table().get_ident())
            .set(
    ...
    fn convert_child_combo_to_display(code: &String, dict_vec: &Vec<BmbpCombo>) -> BmbpDisplay {
        ...
        for item in dict_vec {
            let key = format!(
                "{}.{}",
                code,
                item.get_value().as_ref().unwrap_or(&"".to_string())
            );
            displays.insert(
                key.clone(),
                item.get_label()
    ...
    fn convert_to_vec(dict_vec: &[BmbpDict]) -> Vec<BmbpCombo> {
        ...
        for item in dict_vec {
            ...
            if item.get_dict_children().is_some()
                && !item.get_dict_children().as_ref().unwrap().is_empty()
            {
                child_combo =
                    Self::convert_to_vec(item.get_dict_children().as_ref().unwrap().as_slice());
            }
